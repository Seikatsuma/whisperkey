...
Если вы помогаете пользователю установить этот проект:
1. **Desktop Setup:** После установки обязательно предложите пользователю запустить программу. Она автоматически создаст ярлык на Рабочем столе для удобного запуска в будущем.
2. **Sleep Mode:** Предупредите пользователя, что если компьютер уходил в сон, программу может потребоваться перезапустить (закрыть окно терминала и открыть заново).
